...
def load():
    f = [p for p in Path("buckets").glob("*.pt")]
    f = sorted(f,key=lambda x:int(x.name.split(".pt")[0]))
    data = torch.load(f[0])
    os.remove(f[0])
    return data
...
class LSHSelfAttention(nn.Module, EfficientAttentionMixin):
    ...
    def _hash_vectors(self, vectors, num_hashes, attention_mask, increase_num_buckets=False):
        ...
        # TODO
        random_rotations = load().to(vectors.device).to(vectors.dtype)
...
def save_data(data):
    from pathlib import Path
    try:
        n = sorted([int(p.name.split(".pt")[0]) for p in Path("jilu").glob("*.pt")])[-1]+1
    except:
        n = 0
    torch.save(data,f"jilu/{n}.pt")

...
from paddlenlp.transformers.nezha.modeling import ACT2FN
...
def torchlayernorm(x,weight,bias):
    x = torch.from_numpy(x.cpu().numpy()).to(device)
    weight = torch.from_numpy(weight.clone().cpu().numpy()).to(device)
    bias = torch.from_numpy(bias.clone().cpu().numpy()).to(device)
...
def _len_and_dim_norm(vectors,attention_head_size):
    ...
    """
    vectors = torch.from_numpy(vectors.cpu().numpy()).to(device)
...
def torchlogsumexp(query_key_dots, axis=-1, keepdim=True):
    x = torch.from_numpy(query_key_dots.cpu().numpy()).to(device)
...
def _apply_chunking_to_forward(
...
def _get_least_common_mult_chunk_len(pd_config):
    ...
    if len(attn_types_set) == 1 and attn_types[0] == "lsh":
        ...
        return np.lcm(
            pd_config.lsh_attn_chunk_length, pd_config.local_attn_chunk_length
        )
    else:
...
def _get_min_chunk_len(pd_config):
    ...
    if len(attn_types_set) == 1 and attn_types[0] == "lsh":
        ...
        return min(pd_config.lsh_attn_chunk_length, pd_config.local_attn_chunk_length)
    else:
        raise NotImplementedError(
            f"Only attn layer types 'lsh' and 'local' exist, but `pd_config.attn_layers`: {pd_config.attn_layers}. Select "
            "attn layer types from ['lsh', 'local'] only."
...
class AxialPositionEmbeddings(nn.Layer):
    ...
    def forward(self, position_ids):
        ...
        broadcasted_weights = [
            weight.expand(
                shape=(batch_size,)
                + tuple(self.axial_pos_shape)
                + tuple(weight.shape[-1:])
            )
        ...
        if self.training is True:
            ...
            position_encodings = paddle.concat(
                [
                    paddle.index_select(
                        position_encodings[i], index=position_ids[i], axis=0
                    ).unsqueeze(0)
...
class PositionEmbeddings(nn.Layer):
    ...
    def __init__(self, pd_config):
        ...
        self.embedding = nn.Embedding(
            pd_config.max_position_embeddings, pd_config.hidden_size
        )
...
class ReformerEmbeddings(nn.Layer):
    ...
    def __init__(self, pd_config):
        ...
        self.word_embeddings = nn.Embedding(pd_config.vocab_size, pd_config.hidden_size)
        self.position_embeddings = (
            AxialPositionEmbeddings(pd_config)
            if pd_config.axial_pos_embds
            else PositionEmbeddings(pd_config)
        )
    ...
    def _look_adjacent(self, vectors, num_chunks_before, num_chunks_after):
        ...
        for i in range(-num_chunks_before, num_chunks_after + 1):
            if i == 0:
                ...
                slices.append(
                    paddle.concat([vectors[:, :, i:], vectors[:, :, :i]], axis=2)
                )
...
class LSHSelfAttention(nn.Layer, EfficientAttentionMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if do_cached_attention:
            ...
            # get query vector
            if Strategy[1]:
                query_vectors = torchdense(hidden_states,self.query_key.weight,None)
            else:
            ...
            if past_buckets is not None:
                ...
                query_vectors = paddle.tile(
                    query_vectors.unsqueeze(2), repeat_times=[1, 1, num_hashes, 1, 1]
                )
            else:
                ...
                if Strategy[1]:
                    query_vectors = torchdense(key_value_hidden_states,self.query_key.weight,None)
                else:
                    query_vectors = self.query_key(key_value_hidden_states)
                
                if Strategy[1]:
                    value_vectors = torchdense(key_value_hidden_states,self.value.weight,None)
                else:
            ...
            query_vectors = None
            if Strategy[1]:
                query_key_vectors = torchdense(hidden_states,self.query_key.weight,None)
            else:
                query_key_vectors = self.query_key(hidden_states)
            
            if Strategy[1]:
                value_vectors = torchdense(hidden_states,self.value.weight,None)
            else:
        ...
        # scale key vectors
        if Strategy[3]:
            key_vectors = _len_and_dim_norm(query_key_vectors,self.attention_head_size)
        else:
        ...
        if not do_standard_self_attention or (
            ...
            if num_hashes > 1:
                ...
                # 60
                save_data(probs_vectors)
                
                # 61
                save_data(out_vectors * probs_vectors)
                if Strategy[2]:
                    out_vectors = torchsum(out_vectors * probs_vectors, axis=2)
                else:
    ...
    def _query_per_attn_head(self, hidden_states):
        ...
        query_key_vectors = einsum("balh,ahr->balr", hidden_states, per_head_query_key)
        ...
        if isinstance(self.num_buckets, int) or len(self.num_buckets) == 1:
            ...
            for bucket_factor in self.num_buckets:
                ...
                if buckets is None:
                    ...
                    buckets = buckets + (
                        cur_product * paddle.argmax(rotated_vectors_factor, axis=-1)
                    )
    ...
    def _attend(
        ...
        if not do_standard_self_attention:
            ...
            query_bucket_idx = (query_key_dots.shape[-1] - 1) * paddle.ones_like(
                query_key_dots
            )[:, :, :, -1]
        ...
        if attention_mask is not None:
            ...
            if not do_standard_self_attention:
                ...
                attention_mask = paddle.index_sample(
                    attention_mask, new_key_indices
                ).reshape(key_indices.shape)
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        bucket_idx_batch_offset = sequence_length * (
            batch_size
            * paddle.arange(relevant_bucket_idx_chunk.shape[-1])
    ...
    def _expand_to_indices_in_relevant_chunk(self, indices, sequence_length):
        ...
        chunk_sequence_indices = expanded_start_indices + paddle.arange(
            total_chunk_size
        ).unsqueeze(0).expand(shape=[indices.shape[0], total_chunk_size])
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        expanded_idxs = paddle.tile(
            idxs.unsqueeze(-2), repeat_times=[1, 1, self.attention_head_size, 1]
...
class ReverseSort(PyLayer):
    ...
    def forward(ctx, out_vectors, logits, sorted_bucket_idx, undo_sorted_bucket_idx):
        ...
        with paddle.no_grad():
            ...
            expanded_undo_sort_indices = undo_sorted_bucket_idx.unsqueeze(-2).expand_as(
                out_vectors
            )
            out_vectors = (
                paddle.index_sample(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if use_cache and past_buckets_states[1] is not None:
            ...
            if Strategy[1]:
                query_vectors = torchdense(hidden_states,self.query.weight,None)
                key_vectors = torchdense(key_value_hidden_states,self.key.weight,None)
                value_vectors = torchdense(key_value_hidden_states,self.value.weight,None)
            else:
            ...
            if Strategy[1]:
                query_vectors = torchdense(hidden_states,self.query.weight,None)
                key_vectors = torchdense(hidden_states,self.key.weight,None)
                value_vectors = torchdense(hidden_states,self.value.weight,None)
            else:
                query_vectors = self.query(hidden_states)
                ...
                value_vectors = self.value(hidden_states)
            # 2
            save_data(query_vectors)
            # 3
            save_data(key_vectors)
            # 4
            save_data(value_vectors)
            print("save query_vectors")
...
class ReformerSelfOutput(nn.Layer):
    def __init__(self, pd_config):
        ...
        self.dense = nn.Linear(all_head_size, pd_config.hidden_size, bias_attr=False)

    def forward(self, hidden_states):
        # 17
        save_data(hidden_states)
...
class ReformerAttention(nn.Layer):
    ...
    def forward(
        ...
        if Strategy[0]:
            hidden_states = torchlayernorm(hidden_states,self.layer_norm.weight,self.layer_norm.bias)
        else:
        ...
        if use_cache:
            ...
            if past_buckets_states[self.layer_id][1] is None:
                ...
                past_states = paddle.concat(
                    [past_buckets_states[self.layer_id][1], hidden_states], axis=1
                )
...
class ReformerFeedForwardDense(nn.Layer):
    def __init__(self, pd_config):
        ...
        self.dense = nn.Linear(pd_config.hidden_size, pd_config.feed_forward_size)

    def forward(self, hidden_states):
        if Strategy[1]:
            hidden_states = torchdense(hidden_states,self.dense.weight,self.dense.bias)
        else:
            hidden_states = self.dense(hidden_states)
        hidden_states = F.dropout(hidden_states, p=self.dropout, training=self.training)
...
class ReformerFeedForwardOutput(nn.Layer):
    def __init__(self, pd_config):
        ...
        self.dense = nn.Linear(pd_config.feed_forward_size, pd_config.hidden_size)
...
class ChunkReformerFeedForward(nn.Layer):
    ...
    def forward(self, attention_output):
        return self.forward_chunk(attention_output)
...
def save_data(data):
    ...
    try:
        n = sorted([int(p.name.split(".pd")[0]) for p in Path("jilu").glob("*.pd")])[-1]+1
    except:
        n = 0
    paddle.save(data,f"jilu/{n}.pd")

def save_buckets(data):
    from pathlib import Path
    try:
        n = sorted([int(p.name.split(".pt")[0]) for p in Path("buckets").glob("*.pt")])[-1]+1
    except:
        n = 0
    torch.save(torch.from_numpy(data.numpy()),f"buckets/{n}.pt")
...
class ReformerLayer(nn.Layer):
    ...
    def forward(
        ...
        with paddle.no_grad():
            ...
            # Y_1 = X_1 + f(X_2)
            # 19
            save_data(prev_attn_output)
            attn_output = prev_attn_output + attn_output
            # 20
            save_data(attn_output)
            # free memory
...
class ReformerEncoder(nn.Layer):
    def __init__(self, pd_config):
        ...
        self.layers = nn.LayerList(
            [ReformerLayer(pd_config, i) for i in range(pd_config.num_hidden_layers)]
        )
...
class ReformerOnlyLMHead(nn.Layer):
    def __init__(self, pd_config):
        ...
        self.bias = self.create_parameter(shape=(pd_config.vocab_size,), is_bias=True)
...
class ReformerPreTrainedModel(PretrainedModel):
    ...
    def tie_weights(self):
        ...
        """
        if (
            hasattr(self, "get_output_embeddings")
            and hasattr(self, "get_input_embeddings")
            and self.pd_config.get("tie_word_embeddings", False)
        ):
...
class ReformerModel(ReformerPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states = (
            ...
            else self.pd_config.output_hidden_states
        )
        return_dict = (
    ...
    def _pad_to_mult_of_chunk_length(
        ...
        if attention_mask is not None:
            pad_attention_mask = paddle.zeros(
                shape=[input_shape[0], padding_length], dtype=attention_mask.dtype
            )
            ...
            attention_mask = paddle.concat(
                [
                    ...
                    paddle.zeros(
                        shape=(input_shape[0], padding_length), dtype=paddle.int64
                    ),
                ],
                axis=-1,
            )
...
class ReformerModelWithLMHead(ReformerPreTrainedModel):
    def __init__(self, reformer):
        ...
        assert (
            "local" not in self.pd_config.attn_layers
            or pd_config.local_num_chunks_after == 0
        ...
        assert (
            "lsh" not in self.pd_config.attn_layers
            or pd_config.lsh_num_chunks_after == 0
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(
                logits.reshape(shape=[-1, self.pd_config.vocab_size]), labels.flatten()
            )
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.pd_config.problem_type == "regression":
                ...
                loss_fct = nn.CrossEntropyLoss()
                loss = loss_fct(
                    logits.reshape(shape=[-1, self.num_classes]), labels.flatten()
                )
...
class ReformerClassificationHead(nn.Layer):
    ...
    def __init__(self, pd_config):
        ...
        classifier_dropout = (
            pd_config.classifier_dropout
            if pd_config.classifier_dropout is not None
            else pd_config.hidden_dropout_prob
        )
...
class ReformerForQuestionAnswering(ReformerPreTrainedModel):
    def __init__(self, reformer, num_classes):
        ...
        self.qa_outputs = nn.Linear(2 * pd_config.hidden_size, pd_config.num_labels)

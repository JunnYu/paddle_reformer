...
def convert_pytorch_state_dict_to_flax(pt_state_dict, flax_model):
    ...
    remove_base_model_prefix = (
        ...
        in set([k.split(".")[0] for k in pt_state_dict.keys()])
    )
    add_base_model_prefix = (flax_model.base_model_prefix in flax_model.params) and (
        flax_model.base_model_prefix
        not in set([k.split(".")[0] for k in pt_state_dict.keys()])
    )
    ...
    for pt_key, pt_tensor in pt_state_dict.items():
        ...
        if (
            ...
            and pt_tuple_key not in random_flax_state_dict
        ):
            # conv layer
            pt_tuple_key = pt_tuple_key[:-1] + ("kernel",)
            pt_tensor = pt_tensor.transpose(2, 3, 1, 0)
        elif (
            pt_tuple_key[-1] == "weight" and pt_tuple_key not in random_flax_state_dict
        ):
            # linear layer
            pt_tuple_key = pt_tuple_key[:-1] + ("kernel",)

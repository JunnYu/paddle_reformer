...
class FlaxTopPLogitsWarper(FlaxLogitsWarper):
    ...
    def __call__(
        ...
        score_mask |= jax.ops.index_update(
            jnp.roll(score_mask, 1), jax.ops.index[:, 0], True
        )

        # min tokens to keep
        score_mask = jax.ops.index_update(
            score_mask, jax.ops.index[:, : self.min_tokens_to_keep], True
        )
...
class FlaxTopKLogitsWarper(FlaxLogitsWarper):
    ...
    def __call__(
        ...
        next_scores_flat = jax.ops.index_update(
            next_scores_flat, topk_indices_flat, topk_scores_flat
        )
...
class FlaxForcedBOSTokenLogitsProcessor(FlaxLogitsProcessor):
    ...
    def __call__(
        ...
        scores = jnp.where(
            apply_penalty,
            jax.ops.index_update(new_scores, jax.ops.index[:, self.bos_token_id], 0),
            scores,
        )
...
class FlaxForcedEOSTokenLogitsProcessor(FlaxLogitsProcessor):
    ...
    def __call__(
        ...
        scores = jnp.where(
            apply_penalty,
            jax.ops.index_update(new_scores, jax.ops.index[:, self.eos_token_id], 0),
            scores,
        )
    ...
    def __call__(
        ...
        scores = jnp.where(
            ...
            jax.ops.index_update(
                scores, jax.ops.index[:, self.eos_token_id], -float("inf")
            ),
            scores,
        )

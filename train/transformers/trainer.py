...
class Trainer:
        ...
        if os.path.isfile(os.path.join(checkpoint, "optimizer.pt")) and os.path.isfile(
            ...
            if is_torch_tpu_available():
                ...
                optimizer_state = torch.load(
                    os.path.join(checkpoint, "optimizer.pt"), map_location="cpu"
                )
                with warnings.catch_warnings(record=True) as caught_warnings:
                    lr_scheduler_state = torch.load(
                        os.path.join(checkpoint, "scheduler.pt"), map_location="cpu"
                    )
                reissue_pt_warnings(caught_warnings)
                ...
                reissue_pt_warnings(caught_warnings)
                if self.use_amp and os.path.isfile(
                    os.path.join(checkpoint, "scaler.pt")
                ):
                    self.scaler.load_state_dict(
                        torch.load(os.path.join(checkpoint, "scaler.pt"))
                    )
    ...
    def store_flos(self):
        ...
        if self.args.local_rank != -1:
            self.state.total_flos += (
                distributed_broadcast_scalars([self.current_flos]).sum().item()
            )
            self.current_flos = 0
        else:
    ...
    def init_git_repo(self):
        ...
        if not self.args.should_save:
            return
        use_auth_token = (
            True if self.args.push_to_hub_token is None else self.args.push_to_hub_token
        )

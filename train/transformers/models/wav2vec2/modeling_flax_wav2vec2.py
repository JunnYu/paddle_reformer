...
class FlaxConvWithWeightNorm(nn.Module):
    ...
    def setup(self):
        ...
        self.weight_v = self.param(
            "weight_v", jax.nn.initializers.he_normal(dtype=self.dtype), weight_shape
        )
...
class FlaxWav2Vec2Module(nn.Module):
    ...
    def setup(self):
        self.feature_extractor = FlaxWav2Vec2FeatureExtractor(
            self.config, dtype=self.dtype
        )
    ...
    def __call__(
        ...
        if attention_mask is not None:
            ...
            attention_mask = jax.ops.index_update(
                attention_mask,
                jax.ops.index[jnp.arange(attention_mask.shape[0]), output_lengths - 1],
                1,
            )

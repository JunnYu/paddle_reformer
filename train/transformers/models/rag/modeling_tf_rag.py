...
class TFRagModel(TFRagPreTrainedModel):
    ...
    def call(
        ...
        **kwargs,
        ...
        n_docs = (
            inputs["n_docs"] if inputs["n_docs"] is not None else self.config.n_docs
        )
...
class TFRagTokenForGeneration(TFRagPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def call(
        ...
        **kwargs,  # needs kwargs for generation
        ...
        inputs["do_marginalize"] = (
            inputs["do_marginalize"]
            if inputs["do_marginalize"]
            else self.config.do_marginalize
        )
        inputs["reduce_loss"] = (
            inputs["reduce_loss"] if inputs["reduce_loss"] else self.config.reduce_loss
        )

...
class FlaxT5LayerCrossAttention(nn.Module):
    ...
    def setup(self):
        self.EncDecAttention = FlaxT5Attention(
            self.config, has_relative_attention_bias=False, causal=False
        )
        self.layer_norm = FlaxT5LayerNorm(
        ...
        if self.embed_tokens is None:
            self.embed_tokens = nn.Embed(
                ...
                embedding_init=jax.nn.initializers.normal(
                    self.config.init_std, self.dtype
                ),
                dtype=self.dtype,
            )
...
class FlaxT5Module(nn.Module):
    ...
    def setup(self):
        self.shared = nn.Embed(
            ...
            self.config.d_model,
            embedding_init=jax.nn.initializers.normal(
                self.config.initializer_factor * 1.0, self.dtype
            ),
            dtype=self.dtype,
        )
    ...
    @replace_return_docstrings(
        output_type=FlaxSeq2SeqModelOutput, config_class=_CONFIG_FOR_DOC
    )
    def __call__(
        self,
        input_ids=None,
...
class FlaxT5ForConditionalGenerationModule(nn.Module):
    ...
    def setup(self):
        ...
        self.shared = nn.Embed(
            ...
            self.config.d_model,
            embedding_init=jax.nn.initializers.normal(
                self.config.initializer_factor, self.dtype
            ),
        )
